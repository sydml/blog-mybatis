...
document.getElementById("articleCategories").onchange = function () {
    ...
    if (categoryId === "") {
        ...
        var url = "http://localhost/api/article/list/sort/" + categoryId;
    }
    // 填充博文分类信息
    $.ajax({
        url: url,
...
function edirotShowOrHide() {
    if($('#updateEditorMode').is(':checked')) {
        $('#richTextEditor').hide();
        // $('#richTextEditor').hide();
        $('#updateMarkdownEditor').show();
        tinyMCE.activeEditor.setContent();
    }else{
        $('#updateMarkdownEditor').hide();
        // $('#richTextEditor').show();
        $('#richTextEditor').show();
        tinyMCE.activeEditor.setContent($('#updateArticleContent').val());
    }
}
...
$('#confirmBtn').click(function () {
    ...
    $.ajax({
        type: "DELETE",
        url: "http://localhost/admin/article/" + id,
        success: function () {
            // 刷新页面
            location.reload();
        }
...
$('#updateBtn').click(function () {
    ...
    $.ajax({
        type: "PUT",
        url: "http://localhost/admin/article/" + articleId,
        dataType: "json",
...
$('#addArticleBtn').click(function () {
    ...
    if(articleContent==null || articleContent=="") {
        articleContent = $('#addMarkDownArticleContent').val();
    }
    // var editorMode = $('#editorMode').val();
    ...
    $.ajax({
        type: "POST",
        url: "http://localhost/admin/article/",
        dataType: "json",
        contentType: "application/json;charset=utf-8",
        data: JSON.stringify(article),
        success: function () {
